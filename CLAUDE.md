...
**版本号格式**：`v[大版本].[小版本].[内部迭代四位数]`
- 大版本（X）：重大架构变更时递增
- 小版本（Y）：对外发布时递增（如 v0.1 → v0.2）
- 内部迭代（NNNN）：每次 CHANGELOG 条目全局递增，**不随大/小版本更新而清零**；对外发布时隐藏此字段，只展示 vX.Y
- 示例：当前 v0.1.0027，对外发布 v0.1 后，下一版本为 v0.2.0028
