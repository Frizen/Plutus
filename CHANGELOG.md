...
### 变更内容
- 「一键创建测试表格」从「从零建表 + 逐字段创建」改为「从固定模板复制」：通过飞书 Drive Copy API 复制预制模板（已含正确字段结构与格式），省去并发建字段步骤，流程更简洁、结构更可控
- 移除 `createBitableApp`、`createExpenseFields` 两个方法，新增 `getRootFolderToken`、`copyBitableFromTemplate` 替代

## v0.1.0028 — 修复测试表格链接权限
**日期**：2026-03

### 变更内容
- 修复「一键创建测试表格」后，链接权限未正确开放给互联网所有人的问题：`setPublicEditPermission` 中 `link_share_entity` 由 `tenant_readable`（仅组织内可读）改为 `anyone_editable`（互联网任何人可编辑）

## v0.1.0027 — 稳定性与体验优化
**日期**：2026-03

### 变更内容
- **分类透明度**：`ExpenseRecord` 新增 `needsPhase2` 字段，Phase 2（GLM 详情识别）未完成时标记为待定；记录列表对分类尚未确定的条目显示灰色「分类待定」提示，取代原先的静默失败
- **网络超时兜底**：`FeishuBitableService` 所有 URLRequest 统一设置超时——Token 请求 15s，记录读写 / 建表请求 20s，防止网络异常时长时间挂起
- **友好错误提示**：重写 `FeishuError.errorDescription`，常见错误（凭证不正确、无访问权限、请求频率超限等）展示中文说明，不再向用户暴露技术细节
- **向导状态重构**：`WizardFeishuPage` 的 7 个独立 `@State` 变量合并为 `FeishuSetupState` 值类型，状态转换逻辑集中到结构体内的 `mutating` 方法，提升可读性

## v0.1.0026 — Code review 修复
**日期**：2026-03
...
- 合并 v0.1.0023–v0.1.0025 散碎版本，CHANGELOG 按 commit 粒度整理

---

## v0.1.0025 — 配置向导（Setup Wizard）
**日期**：2026-03
...
## v0.1.0016 — Phase 2 超时修复：图片不再重复编码
**日期**：2026-03
...
## v0.1.0015 — AppShortcut 自动注册，快捷指令无需手动创建
**日期**：2026-03
